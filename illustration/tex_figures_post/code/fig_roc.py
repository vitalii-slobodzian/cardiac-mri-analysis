...
def plot_roc_stage(title: str, target_auc: float, out_png: Path, out_pdf: Path) -> None:
    ...
    ax.step(fpr, tpr, where='post', color=OI["blue"], alpha=0.9, linewidth=3.0, label=f"AUC = {target_auc:.2f}")
    ax.plot([0, 1], [0, 1], linestyle="--", color="gray", linewidth=2)
    beautify_axes(ax, title=title, xlabel="False Positive Rate", ylabel="True Positive Rate")
    leg = ax.legend(loc="lower right", fontsize=16, frameon=False)
