...
def plot_sota(out_png: Path, out_pdf: Path) -> None:
    ...
    axes[0].plot(x, ed_lv, marker="o", color=OI["blue"], linewidth=3.0, label="LV")
    axes[0].plot(x, ed_rv, marker="s", color=OI["vermillion"], linewidth=3.0, label="RV")
    axes[0].plot(x, ed_myo, marker="^", color=OI["bluish_green"], linewidth=3.0, label="Myocardium")
    axes[0].set_ylim(0.85, 1.0)
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(methods, rotation=45, ha="right")
    beautify_axes(axes[0], title="ED", ylabel="Dice")

    axes[1].plot(x, es_lv, marker="o", color=OI["blue"], linewidth=3.0, label="LV")
    axes[1].plot(x, es_rv, marker="s", color=OI["vermillion"], linewidth=3.0, label="RV")
    axes[1].plot(x, es_myo, marker="^", color=OI["bluish_green"], linewidth=3.0, label="Myocardium")
    axes[1].set_ylim(0.85, 1.0)
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(methods, rotation=45, ha="right")
    beautify_axes(axes[1], title="ES")
