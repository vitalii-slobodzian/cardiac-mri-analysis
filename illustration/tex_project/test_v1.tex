...
\begin{abstract}
This study introduces an explainable deep learning framework to address critical gaps in automated cardiac MRI analysis, where existing methods often falter in segmentation accuracy, classification of similar pathologies, and decision transparency. Our objective was to develop a robust pipeline that enhances diagnostic precision using the public ACDC dataset, containing 2,978 short-axis cine-MRI scans from 150 patients. We propose a multi-stage segmentation method that employs U-Net architectures with pre-trained ResNet-50 and ResNet-34 encoders for anatomical localization and delineation. For pathology detection, a cascade of custom convolutional neural network classifiers was developed to differentiate five cardiac conditions (DCM, HCM, MINF, ARV, and NOR). A novel interpretation module translates these classifications into clinically relevant metrics. The segmentation method achieved state-of-the-art Dice coefficients of 0.974 for the left ventricle, 0.947 for the right ventricle, and 0.920 (systole) for the myocardium. The cascade classifier attained an overall accuracy of 97\%, significantly outperforming standard single-stage architectures (72–84\%) by resolving inter-class confusion. The interpretation model visualizes key functional indicators, such as ejection fraction and wall thickness, demystifying the "black box" of AI-driven diagnosis.
\end{abstract}
...
\begin{itemize}
    \item A multi-stage method is proposed for segmenting the heart region into the right ventricle (RV), left ventricle (LV), and myocardium. It differs from existing approaches by combining specialized deep learning models for localization and segmentation, followed by post-processing for contour refinement, and allows for improving segmentation accuracy compared to baseline models.
    \item A cascade classification method is introduced for pathologies—dilated cardiomyopathy (DCM), hypertrophic cardiomyopathy (HCM), myocardial infarction with altered left ventricular ejection fraction (MINF), and abnormal right ventricle (ARV)—on MRI scans. This method differs from existing ones by applying a cascade of DL models to segmented MRI scans, achieving higher disease classification accuracy for the four pathologies under consideration.
    \item A method for interpreting the decisions obtained by DL is presented, differing from existing approaches by explaining the features used in medical practice, which makes the decisions transparent and comprehensible.
\end{itemize}
...
Each dataset formalized by equations (3)–(5) localizes and centers images/masks around one structure, adding a 15\% margin. After segmentation, images must revert to original dimensions for final validation, possibly causing detail loss. To address this, Gaussian smoothing is utilized as suggested in our previous work \cite{Slobodzian:2024}—balancing quality and computational cost better than other filters. The parameter \(\sigma \) is automatically chosen via linear regression, fitting optimal \(\sigma \) to each image size.
...
Dataset \({D_6}\) is derived from the ACDC dataset, containing systolic and diastolic images for 150 patients in five groups. Some have fewer than 21 short-axis slices; thus, image augmentation is performed by duplicating adjacent slices.
...
The third classification stage also achieved 1.0 in Accuracy, Recall, and \({F_1}\)-score, effectively separating HCM from other LV pathologies such as MINF and DCM. This emphasizes the model’s ability to recognize specific heart pathologies even under conditions of subtle inter-class differences.
...
The original ACDC dataset is licensed under the MIT License and can be found at: \url{https://www.creatis.insa-lyon.fr/Challenge/acdc/databases.html}. The full, pre-processed dataset containing the specific image crops and segmentation masks generated and used for training and testing in this study is available at the following repository: \url{https://drive.google.com/drive/folders/1qBpZR2LvrWwW70OLAJbOx74eWBlxtkA2?usp=sharing} (2025).

\section*{Code availability}
The source code is available at: \url{https://github.com/vitalii-slobodzian/cardiac-mri-analysis/tree/main} (2025).
...
\textbf{Compliance with ethical standards:} This research study was conducted retrospectively using human subject data made available in open access. Ethical approval was not required as confirmed by the license attached with the open-access data.
