...
% For Automatic Cross-Referencing
% \usepackage{xr}
% \externaldocument[S-]{supply}
...
To address the black box issue, existing approaches focus on enhancing AI transparency through interpretation and visualization methods \cite{Shick:2024}. \Rone{\revtag{1}{1} While general-purpose explainability techniques like Grad-CAM or SHAP have been applied to medical imaging, their outputs often highlight pixel-level importance, which may not directly translate into clinically actionable insights. A growing consensus, articulated in reviews like Salih et al. \cite{Salih:2023}, emphasizes that for AI to be truly adopted, its explanations must align with the established diagnostic frameworks and quantitative metrics used by clinicians. Our work directly addresses this need by moving beyond saliency maps to produce interpretations grounded in familiar clinical indices such as ejection fractions and standardized wall-thickness models, thereby bridging the gap between algorithmic output and clinical practice.}
...
\begin{itemize}
    \item \Rone{\revtag{1}{1} \textbf{Hierarchical segmentation with structure specialization.} While multi-stage segmentation is an established paradigm, as seen in works like Wang et al. \cite{Wang:2023b}, our novelty lies in the systematic decomposition of the task into six distinct, structure-specific models (three for localization and three for contouring). This specialized approach allows each model to learn the unique morphological features of its target (e.g., the crescent shape of the RV vs. the circular LV), leading to consistent performance gains, particularly for the challenging RV and the myocardium in the end-systolic phase, as demonstrated in our ablation studies and comparisons with single-stage baselines\cite{Ronneberger:2015,Hu:2023,daSilva:2024}.}
    ...
    \item \Rone{\revtag{1}{1} \textbf{Interpretation aligned with practice.} Our primary contribution to interpretability is not the development of a new explainability algorithm but the creation of an end-to-end pipeline that translates opaque model outputs into a dashboard of clinically canonical metrics. We derive LV/RV volumes, ejection fractions, volume-to-mass ratios, and AHA 17-segment wall-thickness maps consistent with imaging guidelines\cite{Szabo:2022,Watanabe:2022}. This provides study-level summaries that are immediately amenable to clinician review and verification, fostering trust and facilitating adoption.}
\end{itemize}
...
\Rone{\revtag{1}{2} Hybrid segmentation techniques that integrate classical and DL approaches are gaining traction. For instance, combining CNN-based preprocessing with active contour models leverages the strengths of both methods, achieving precise boundary detection. Similarly, ensemble learning methods incorporating multiple DL models can enhance segmentation accuracy by addressing different aspects of the image-processing task. These hierarchical or two-stage pipelines, some incorporating attention mechanisms, have shown promise in refining coarse initial segmentations \cite{Wang:2023b,Patil:2023}. Wang et al. \cite{Wang:2023b}, for instance, employ a two-stage approach that first localizes the whole heart and then segments specific substructures, demonstrating the power of a coarse-to-fine strategy. Patil et al. \cite{Patil:2023} focus on creating domain-shift invariant frameworks to improve generalization across different scanner types, a critical challenge in real-world deployment. These hybrid strategies, while effective, often demand more significant computational resources and complex model tuning. Our work builds on this hierarchical principle but distinguishes itself by dedicating separate, specialized models to each cardiac structure, a level of granularity that proves beneficial for anatomically complex regions like the right ventricle. Specialized models tailored to specific cardiac regions have shown improved accuracy by focusing on unique anatomical features \cite{Wehbe:2023}.}
...
Dataset \({D_6}\) is derived from the ACDC dataset, containing systolic and diastolic images for 150 patients in five groups. Some have fewer than 21 short-axis slices; thus, image augmentation is performed by duplicating adjacent slices. This augmentation occurs only within patient-level training folds to prevent data leakage.
...
\Rone{\subsubsection*{Methodological Policy on Papillary Muscles and Trabeculations}}
\Rone{\revtag{1}{3} For the calculation of all volumetric and mass-based clinical indices derived from the segmentation masks, a consistent anatomical policy was adopted. Papillary muscles and trabeculations were methodologically excluded from the myocardial mass and were considered part of the left ventricular (LV) blood pool. This approach was chosen to ensure consistency and comparability with prevalent clinical reporting standards, which often favor this convention to reduce inter-observer variability in mass and volume quantification. The impact of this methodological choice is further quantified and discussed in the Discussion section.}
...
\subsubsection*{Evaluation, Uncertainty, and Calibration}
...
To objectively situate our method's performance within the current research landscape, we conducted a comprehensive comparison against several state-of-the-art techniques on the ACDC challenge dataset. The quantitative results, benchmarked using the Dice coefficient for both end-diastolic (ED) and end-systolic (ES) phases, are summarized in Table~\ref{tab:segmentation_comparison_sota}.

Our hierarchical 2D approach demonstrates a highly competitive and balanced performance profile across all cardiac structures. For the Left Ventricle (LV), our method achieves a Dice score of 0.974 in ED, closely rivaling top-performing 2D and 3D models like Zhang et al. \cite{Zhang:2022} (0.976) and Benameur et al. \cite{Benameur:2025} (0.978). A key strength of our approach is its exceptional performance in segmenting the Right Ventricle (RV), traditionally a challenging task due to its complex crescent shape and lower blood-myocardium contrast. With Dice scores of 0.947 (ED) and 0.915 (ES), our method is on par with or exceeds the results of leading competitors in this specific area.

Perhaps most notably, our pipeline excels in segmenting the myocardium during the ES phase, achieving a Dice score of 0.920. This is the highest score among the compared methods and is particularly significant because accurate ES myocardium delineation is critical for functional assessments like wall thickening, yet it is notoriously difficult due to the thinness and dynamic motion of the muscle wall at peak contraction. This result underscores the efficacy of our structure-specific models, which can learn the nuanced features of each anatomical component under different physiological states.

\Rone{\revtag{1}{4} The comparison in Table~\ref{tab:segmentation_comparison_sota} includes methods with diverse architectures, such as the 3D U-Net employed by Benameur et al. \cite{Benameur:2025}.}

\begin{table}[ht]
\centering
\caption{\Rone{\revtag{1}{4} Comparison of segmentation results with the state of the art using the Dice coefficient. Our method is benchmarked against other high-performing approaches on the ACDC dataset. Numbers in \textbf{bold} show the highest scores for each category. Methods marked with (*) employ 3D architectures, leveraging inter-slice information.}}
...
Zhang et al. \cite{Zhang:2022} & 0.976 & \textbf{0.949} & 0.903 & \textbf{0.950} & 0.916 & 0.918 \\
Benameur et al. (*)\cite{Benameur:2025} & \textbf{0.978} & 0.945 & \textbf{0.905} & 0.948 & \textbf{0.919} & 0.917 \\
Ours & 0.974 & 0.947 & 0.896 & 0.940 & 0.915 & \textbf{0.920} \\
\bottomrule
\end{tabular}
\end{table}

\Rone{\revtag{1}{4} While 3D models inherently leverage inter-slice spatial context to ensure volumetric smoothness, often leading to excellent results, they typically come with a higher computational and memory cost. Our 2D-based pipeline, while computationally more lightweight, proves to be a powerful alternative by achieving comparable accuracy and even outperforming in certain demanding sub-tasks. It is also crucial to contextualize results from different imaging modalities. While studies like Zhu et al. \cite{Zhu:2023} report exceptional LV segmentation on myocardial perfusion SPECT images, a direct numerical comparison is not appropriate due to fundamental differences in image resolution, contrast, and underlying physics between SPECT and cine MRI. We therefore focus our benchmark on methods validated on the same modality and dataset.}

Summarizing the overall performance, our proposed method achieves an average Dice coefficient of 0.932 across all structures and phases. This represents a tangible improvement over many established methods, such as those by da Silva et al. \cite{daSilva:2024} (0.913) and Bourfiss et al. \cite{Bourfiss:2023} (0.910). The visual comparison provided in Figure~\ref{fig:sota_dice_comparison} further illustrates the competitive standing of our approach.
...
In conclusion, the results strongly indicate that our multi-stage pipeline, which combines specialized localization, decomposition, and postprocessing, is not only highly accurate but also robust, stable across the cardiac cycle, and particularly effective for challenging anatomical regions, making it a promising tool for clinical and research applications.
...
To independently assess the clinical quality of the generated segmentations, the results were validated by a practicing cardiologist using a custom-developed software application, the interface of which is shown in Supplementary Figure~S1 in Appendix A. This tool facilitates a rigorous side-by-side comparison by enabling dynamic adjustments to image contrast and brightness, as well as the opacity of the overlaid masks. To mitigate observer bias, the validation protocol was rigorously designed: masks were anonymized and presented as 'A' and 'B', with their order randomized for each image, and the decision time was recorded. To ensure the robustness of this qualitative assessment, a second, independent cardiologist reviewed a subset of 50 randomly selected cases to establish inter-rater reliability, achieving a Cohen's \(\kappa\) of 0.85 (95\% CI: 0.78--0.92), indicating substantial agreement. For each comparison, the validating clinician was tasked with selecting one of four definitive options: (i) Mask A has higher accuracy, (ii) Mask B has higher accuracy, (iii) both masks have equally high accuracy with clinically insignificant differences, or (iv) both masks have equally poor accuracy, insufficient for medical conclusions.
...
\subsection*{External Validation and Domain Shift Analysis}
\Rtwo{\revtag{2}{1} To rigorously evaluate the generalizability of our pipeline, the models trained exclusively on the ACDC dataset (Siemens 1.5T/3.0T scanners) were deployed without modification on the unseen, multi-vendor, multi-center M\&Ms dataset. This external validation serves as a stringent test of robustness against the domain shifts inherent in real-world clinical data, which arise from variations in scanner hardware, acquisition sequences, and image reconstruction algorithms. As summarized in Extended Data Table \ref{tab:external_performance}, the model demonstrated commendable, albeit reduced, overall performance, confirming its foundational robustness. The mean Dice scores on the M\&Ms set were 0.91 for the LV, 0.86 for the RV, and 0.85 for the myocardium, a predictable decrease from the internal ACDC test set scores but still indicative of strong generalization capabilities.}

\begin{table}[ht]
  \caption{\Rtwo{\revtag{2}{1} Diagnostic segmentation performance on the internal ACDC test set versus the external M\&Ms validation set. The model was trained only on ACDC data. Performance drop on M\&Ms reflects the domain shift from different scanner vendors and sites. Classification AUROC on M\&Ms is not applicable (N/A) as the challenge did not provide the same pathology labels. Values are mean (95\% CI).}}
  \label{tab:external_performance}
  \centering
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Cohort} & \textbf{LV Dice} & \textbf{RV Dice} & \textbf{MYO Dice} & \textbf{AUROC (cascade)} \\
    \midrule
...
\Rtwo{\revtag{2}{1} To dissect the impact of domain shift more granularly, we stratified the segmentation performance by the specific scanner vendor and model within the M\&Ms dataset, with detailed results presented in Supplementary Table~S1 in Appendix A. This breakdown reveals a clear performance gradient directly correlated with the similarity of the target domain to the training domain. Scanners with technical profiles closely matching the ACDC training data, such as the Philips Achieva 1.5T and other Siemens 1.5T models (Symphony, Avanto), yielded Dice scores nearly on par with the ACDC baseline. For instance, the Philips Achieva maintained high Dice scores of 0.951 (LV-ED) and 0.936 (RV-ED). Performance moderately decreased on Siemens "Tim/Fit" systems, which introduce partial technical variations in coil and reconstruction technologies that create a modest domain gap.}

\Rtwo{\revtag{2}{1} Conversely, a substantial performance degradation was observed on scanners from GE Medical Systems. This drop is attributable to a significant domain gap, driven by several key technical differences. GE scanners often utilize a distinct balanced steady-state free precession (bSSFP) implementation (known as FIESTA), which produces different blood-myocardium contrast and signal characteristics compared to Siemens' TrueFISP sequence used in the ACDC training data. Furthermore, proprietary algorithms for coil signal combination and intensity normalization create different bias-field and intensity profiles that the model has not been trained to handle. Variations in typical imaging parameters (e.g., voxel spacing, slice thickness) and, for the 3.0T scanner, more pronounced B0 field inhomogeneity artifacts, collectively contribute to this pronounced domain shift. These findings underscore a critical point: while our model generalizes well across closely related systems, its performance is sensitive to fundamental differences in imaging physics and software. This highlights the crucial importance of domain adaptation strategies or multi-vendor training datasets for developing universally applicable and reliable clinical tools.}
...
\Rtwo{\revtag{2}{2} While our slice-position--aware refinement module improves segmentation at the ventricular extremes (Table~\ref{tab:apical_basal_performance}), challenges remain.}
...
\Rtwo{\revtag{2}{2} A known challenge in cardiac MRI segmentation is the accurate delineation of apical and basal slices, where anatomical boundaries are often less distinct and partial volume effects are more pronounced. While our method does not incorporate algorithms explicitly designed for these slices, the proposed multi-stage approach inherently improves their segmentation. By first localizing the heart, the model reduces the influence of surrounding tissues and background noise. Subsequently, using separate, specialized networks for the LV, RV, and myocardium allows each model to learn the specific morphological features of its target structure, enhancing sensitivity even in low-contrast regions. Finally, post-processing helps refine contours and remove small, erroneous segmentations common in these challenging edge slices. Our slice-position-aware refinement module with its shape-prior further improves performance, as demonstrated by the stratified results (see Table~\ref{tab:apical_basal_performance}). This hierarchical strategy contributes to a more robust and accurate segmentation across the entire ventricular stack, including the difficult apical and basal regions.}

\Rone{\subsubsection*{Considerations on Papillary Muscle Segmentation and its Clinical Impact}}
\Rone{\revtag{1}{3} A significant methodological consideration in cardiac MRI analysis is the treatment of papillary muscles and trabeculations within the left ventricle. These structures, being embryologically part of the myocardium, present an ambiguity: they can either be included in the myocardial mass or treated as part of the LV blood pool. This choice is not trivial, as it directly impacts the calculation of critical clinical indices, particularly LV mass and, to a lesser extent, ejection fraction. As noted by recent studies, such as Benameur et al. \cite{Benameur:2025}, different conventions can lead to systematic variations in reported values, complicating longitudinal studies and inter-institutional comparisons.}

\Rone{\revtag{1}{3 }Our methodological framework adopts a policy of excluding papillary muscles from the myocardial contours, a convention that aligns with clinical guidelines aimed at maximizing reproducibility. To transparently quantify the impact of this policy, we performed a sensitivity analysis on our test set, recalculating key indices under an alternative policy where papillary muscles were included in the myocardial mass. The results, detailed in Supplementary Table~2 in Appendix A, show a systematic and non-negligible impact: including papillary muscles increased the mean calculated LV mass by approximately 8.3\% (from 124.8 g to 135.2 g) and resulted in a slight decrease in the mean ejection fraction (from 62.1\% to 61.5\%). These findings confirm that a consistent and explicitly stated policy is paramount for the clinical utility of any automated segmentation tool. While our default is exclusion for standardization, the modular nature of our pipeline allows this policy to be adapted to specific institutional protocols if required.}
...
\textbf{Compliance with ethical standards:} We used de-identified, publicly available data from the ACDC and M\&Ms challenges; no new institutional review board approval was required for this retrospective study. Our use of the data complied with the respective terms of each challenge.
