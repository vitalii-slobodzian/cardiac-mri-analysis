...
def plot_dice(out_png: Path, out_pdf: Path) -> None:
    ...
    for ax, phase, data in zip(axes, ["ED", "ES"], [data_ed, data_es]):
        ax.plot(experiments, data["LV"], marker="o", color=OI["blue"], linewidth=2.0, label="LV")
        ax.plot(experiments, data["RV"], marker="s", color=OI["vermillion"], linewidth=2.0, label="RV")
        ax.plot(experiments, data["MYO"], marker="^", color=OI["bluish_green"], linewidth=2.0, label="Myocardium")
